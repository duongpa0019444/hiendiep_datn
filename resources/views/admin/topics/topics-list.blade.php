    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 mt-2 gap-2">
                <!-- Tiêu đề -->
                <h4 class="card-title mb-0">Danh sách Chủ đề bài viết & tin tức</h4>

                <!-- Nhóm nút chức năng -->
                <div class="d-flex flex-wrap gap-2 align-items-center">
                    <!-- Form tìm kiếm + Reset -->
                    <form action="{{ route('admin.topics.filter') }}" method="POST" class="d-flex align-items-center gap-2"
                        id="searchForm">
                        @csrf
                        <input type="hidden" name="limit" id="limit" value="10">
                        <div class="position-relative">
                            <input type="search" name="keyword" class="form-control form-control-sm pe-5"
                                placeholder="Tìm kiếm..." autocomplete="off" value="{{ request('keyword') }}">
                    ...
                    |
                    <!-- Nút thùng rác -->
                    <a href="{{ route('admin.topics.trash') }}" class="btn btn-outline-danger btn-sm d-flex align-items-center gap-1">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            ...
                                                            <li>
                                                                <form
                                                                    ...
                                                                    @method('DELETE')
                                                                    <button type="button"
                                                                        class="dropdown-item text-danger bg-transparent border-0 w-100 text-start btn-delete-topic">
                                                                        <iconify-icon
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function() {
                            Swal.fire('Đã xóa!', 'Chủ đề đã được xóa thành công.', 'success');
                            form.closest('tr').remove();
                        },
                        error: function() {
                            Swal.fire('Lỗi!', 'Không thể xóa chủ đề.', 'error');
                        }
                    });
                }
            });
            ...
            data.forEach(topic => {
                html += `
                    <tr>
                        ...
                        <!-- Ngày tạo -->
                        <td>${moment(topic.created_at).format('DD/MM/YYYY')}</td>
                        ...
                        <td>
                            <div class="btn-group dropstart">
                                ...
                                <ul class="dropdown-menu">
                                    <li>
                                        <a href="/admin/topics/edit/${topic.id}" class="dropdown-item text-warning">
                                            <iconify-icon icon="solar:pen-2-broken" class="me-1"></iconify-icon> Sửa
                                        </a>
                                    </li>
                                    <li>
                                        <form action="/admin/topics/delete/${topic.id}" method="POST">
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            <input type="hidden" name="_method" value="DELETE">
                                            <button type="button" class="dropdown-item text-danger bg-transparent border-0 w-100 text-start btn-delete-topic">
